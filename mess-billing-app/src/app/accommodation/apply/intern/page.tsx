...
export default function InternAccommodationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (financialSupportAmount) formData.append('financialSupportAmount', financialSupportAmount);
      if (uploadedFile) formData.append('file', uploadedFile);
      const res = await fetch('/api/accommodation/apply', { method: 'POST', body: formData });
      ...
      <div className="space-y-8 animate-in fade-in duration-500 slide-in-from-bottom-4">
        <Card className="p-0 overflow-hidden max-w-2xl mx-auto">
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500 font-medium max-w-sm mx-auto mb-8">Your accommodation application has been submitted for review. Track its status from your dashboard.</p>
    ...
    <div className="space-y-6 animate-in fade-in duration-500 slide-in-from-bottom-4">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className="text-sm text-slate-700 font-semibold">Intern Request Form</span>
        </div>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Summer Internship Application</h1>
          ...
          <Card className="p-0 overflow-hidden border border-slate-200/60 shadow-md" hoverEffect={false}>
            ...
            <div className="px-6 py-4 border-b border-slate-100/50 flex items-center gap-3 bg-gradient-to-r from-emerald-50/50 to-transparent">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Summer Intern / Visiting Student</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                ...
                <div>
                  ...
                  <input type="text" value={department} onChange={e => setDepartment(e.target.value)} required className={inputClass} placeholder="e.g. Computer Science" />
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Permanent Address <span className="text-rose-500">*</span></label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Name of Mentor <span className="text-rose-500">*</span></label>
                  <input type="text" value={mentorName} onChange={e => setMentorName(e.target.value)} required className={inputClass} placeholder="Mentor Name" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Mentor Email <span className="text-rose-500">*</span></label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Financial Support (₹/month) <span className="text-slate-400 font-normal normal-case">(Optional)</span></label>
                  <input type="number" value={financialSupportAmount} onChange={e => setFinancialSupportAmount(e.target.value)} className={inputClass} placeholder="Leave blank if none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Guest Category <span className="text-rose-500">*</span></label>
                  <select value={category} onChange={e => setCategory(e.target.value)} required className={inputClass}>
                    ...
                    <option value="A">Category A (Rent: ₹75/day)</option>
                    <option value="B">Category B (Rent: ₹150/day)</option>
                  </select>
                  <p className="text-[11px] text-slate-500 mt-1 font-medium">Cat A: Support &lt; ₹10k/mon. Cat B: Support &gt; ₹10k/mon.</p>
                </div>
              </div>

              {/* Upload */}
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Offer Letter / Proof (PDF/JPG)</label>
                  ...
                  <div className="text-center">
                    ...
                    <div className="flex text-sm text-slate-600 justify-center mt-1">
                      <label className="relative cursor-pointer font-medium text-emerald-600 hover:text-emerald-500">
                        ...
                        <input type="file" className="sr-only" onChange={handleFileChange} accept=".jpg,.jpeg,.png,.pdf" />
                      </label>
                    </div>
                    <p className="text-xs text-slate-500 font-medium mt-0.5">{uploadedFile ? <span className="text-emerald-600 font-bold">{uploadedFile.name}</span> : 'Up to 5MB'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3.5 bg-amber-50/80 border border-amber-200/60 rounded-xl">
                ...
                <p className="text-sm text-amber-800 font-medium">Your application will be reviewed by: Faculty Mentor → HOD → Asst. Registrar → Chief Warden.</p>
              </div>
              ...
              <label className="flex items-start p-3.5 bg-slate-50 rounded-xl border border-slate-200/60 cursor-pointer hover:bg-emerald-50/30 transition-colors">
                ...
                <span className="ml-3 text-sm text-slate-700 font-medium leading-relaxed">
                  I have gone through all rules and regulations of the hostel and will abide by the same during my stay. I agree to vacate the room as and when asked by the Management.
                </span>
              </label>
        ...
        <div className="space-y-4">
          <Card className="p-5 bg-gradient-to-br from-emerald-50/50 to-white border-emerald-100/50" hoverEffect={false}>
            ...
            <div className="space-y-2.5">
              {['Faculty Mentor verifies', 'HOD approves department', 'Asst. Registrar clears', 'Chief Warden allocates room'].map((step, i) => (
          ...
          <Card className="p-5 bg-gradient-to-br from-amber-50/50 to-white border-amber-100/50" hoverEffect={false}>
            ...
            <p className="text-sm text-slate-500 font-medium">Cat A: ₹75/day (support &lt; ₹10k). Cat B: ₹150/day (support &gt; ₹10k). Refundable security deposit required.</p>
          </Card>
        </div>
      </div>
    </div>
