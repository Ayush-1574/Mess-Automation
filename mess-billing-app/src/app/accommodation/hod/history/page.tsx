...
export default function HodHistory() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (res.status === 401) { router.push('/accommodation/reviewer-login?role=HOD'); return; }
      if (res.ok) {
  ...
  const filteredRequests = processedRequests.filter(req => 
    ...
    req.hodStatus.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">HOD Processed History</h1>
              ...
              <tbody className="divide-y divide-slate-100/80">
                {filteredRequests.map(req => (
                  <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`text-[10px] font-extrabold px-3 py-1.5 rounded-lg border ${req.hodStatus === 'APPROVED' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                        {req.hodStatus}
                      </span>
                    </td>
