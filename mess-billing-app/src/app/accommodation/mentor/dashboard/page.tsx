...
export default function MentorDashboard() {
  ...
  const filteredRequests = requests.filter(req => req.session === currentSession);
  const pendingRequests = filteredRequests.filter(req => req.mentorStatus === 'PENDING');
  const processedRequests = filteredRequests.filter(req => req.mentorStatus !== 'PENDING');
  const approvedRequests = processedRequests.filter(req => req.mentorStatus === 'APPROVED');
  const rejectedRequests = processedRequests.filter(req => req.mentorStatus === 'REJECTED');
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-5 border-l-4 border-l-blue-500 shadow-sm relative overflow-hidden" hoverEffect={true}>
          <div className="flex items-center justify-between z-10 relative">
            <div>
              <p className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-1">Session {currentSession}</p>
              <h2 className="text-3xl font-black text-slate-800">{filteredRequests.length}</h2>
            </div>
