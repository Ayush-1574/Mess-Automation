                ...
                <tbody className="divide-y divide-slate-100/80">
                  {processedRequests.map(req => (
                    <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`text-xs font-bold px-3 py-1.5 rounded-lg border ${req.registrarStatus === 'APPROVED' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                          {req.registrarStatus}
                        </span>
                      </td>
                      <td className="p-4 pr-8 text-right space-x-2">
