  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Reviewer Dashboard</h1>
          <p className="text-slate-500 mt-2 font-medium">Manage pending accommodation approvals across all stages</p>
        </div>
                ...
                {requests.map(req => (
                  <tr key={req.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="p-4">
                      ...
                      <div className="text-xs font-semibold text-emerald-600">Category {req.category}</div>
                    </td>
                    ...
                    <td className="p-4 pr-8 text-right space-x-2">
                      <button onClick={() => handleAction(req.id, 'APPROVE')} className="inline-flex items-center px-3 py-1.5 bg-green-50 text-green-700 hover:bg-green-600 hover:text-white rounded-lg text-xs font-bold transition-all border border-green-200 hover:border-green-600">
                      ...
                      <button onClick={() => handleAction(req.id, 'REJECT')} className="inline-flex items-center px-3 py-1.5 bg-red-50 text-red-700 hover:bg-red-600 hover:text-white rounded-lg text-xs font-bold transition-all border border-red-200 hover:border-red-600">
