                        ...
                        <tbody className="divide-y divide-slate-100">
                            {student.feesDeposited.map((f: any) => (
                                <tr key={f.id} className="hover:bg-slate-50/50">
                                    ...
                                    <td className="p-4 text-right font-bold text-teal-700">₹{f.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                                    <td className="p-4 text-center text-slate-600">{new Date(f.paymentDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
