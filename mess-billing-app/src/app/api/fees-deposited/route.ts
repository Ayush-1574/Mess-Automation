...
export async function POST(request: Request) {
    try {
        const { rollNo, sessionId, amount, paymentDate } = await request.json();
        if (!rollNo || !sessionId || amount == null || !paymentDate) {
            return NextResponse.json({ error: 'rollNo, sessionId, amount and paymentDate are required' }, { status: 400 });
        }
