...
// Excel columns: RollNo, Amount, PaymentDate (YYYY-MM-DD), SessionName
export async function POST(request: Request) {
    try {
        ...
        for (const row of jsonData) {
            ...
            if (!rollNo || isNaN(amount) || !paymentDateRaw || !sessionName) {
            ...
            if (isNaN(paymentDate.getTime())) {
                errors.push(`Invalid date for rollNo ${rollNo}: ${paymentDateRaw}`);
                continue;
            }
